function [LHS,sol,stress_outer,U_vec,Omega_vec] = ComputeStressesAndSolidBodyMotionOnly(xc,...
                                                                                        posint,...
                                                                                        ndir,...
                                                                                        ori,...
                                                                                        drag_in,...
    ...
    % This function finds the force on each face of a shape subjected to either a drag, a torque or a background flow in Stokes's regimes
    ...
    % Note that the aggregate is shifted to be at the origin to match Pozrikidis's formulation

    mu = 1; % for now
    DIM = 3;
    number_of_faces_in_a_single_cube = 6;
    LHS_single = zeros(number_of_faces_in_a_single_cube*DIM,number_of_faces_in_a_single_cube*DIM);
    ...
    % For the double layer potential, our domain of interest is x \in (-1,1), y \in (-1,1), z=0
    a = -1;
    b = 1;
    ...
    for kk = 1:Nf
        ...
        elseif flow==2 % rotation
            U_infty = (cross(u_infty',(pos0)))';
        elseif flow==3 % extension
            U_infty = M*pos0';
        else
            U_infty = (cross(u_infty',pos0))' + M*pos0';
        end
        ...
        for ff=1:Nf
            ...
            % the linear system
            [constij,xxij] = single_layer(pos0,myposint,myndir); % this builds the matrices for the single layer
            ...
            % system
            DL = DL + DoubleLayerPotential(ai,bj,myposint,myndir,myori,pos0,u_infty,M,flow);
    ...
    % This is what is needed to recover the Torque
    x_cross = [];
    cross_op = zeros(3);
    for ff=1:Nf
        x_vec = posint(ff,:);
    ...
    x_add(:,1:length(x_cross_m)) = x_cross_m_t; % is this supposed to be there? (thinks so)
    
    LHS = [LHS;x_add];
    
    % Check if the system is diagonally dominant
    if all(2*(abs(diag(LHS))))>=(sum(abs(LHS),2))
    ...
    toc
    endtime = toc;
    sprintf("Time to build Linear System is %f",endtime)
    ...
    % let us try to deal with the rank deficiency issue
    r = rank(LHS,eps);
    tic
    if r==size(LHS,2)
        sol = LHS\RHS;
    ...
    toc
    endtime_solve = toc;
    sprintf("Time to solve Linear System is %f",endtime_solve)
