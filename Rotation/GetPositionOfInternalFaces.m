function [internal_faces,finalndir_internal,finalori_internal] = GetPositionOfInternalFaces(xc, NC)
...
trl = length(toremove);
tokeep = [];
to_remove = []; % I added this to actually get the internal faces
for j=1:6*NC
	keep = 0;
	for i=1:trl
...
%internal_face_pos = posint(toremove,:);
internal_faces = posint(to_remove,:);
finalndir_internal = ndir(to_remove);
finalori_internal = ori(to_remove);
